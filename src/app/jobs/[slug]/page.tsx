    ...
    <div className="flex flex-col lg:flex-row gap-8">
      ...
      <div className="lg:w-80 space-y-6">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 lg:sticky lg:top-24">
          <h3 className="text-lg font-bold text-slate-900 mb-4 text-center">Ready to Apply?</h3>
          <a
            href={job.applyLink}
            target="_blank"
            rel="noopener noreferrer"
            className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-4 rounded-xl flex items-center justify-center gap-2 transition-all shadow-md hover:shadow-lg"
          >
            ...
            <div className="space-y-4">
              {relatedJobs.map((rj) => (
                <Link key={rj.id} href={`/jobs/${rj.slug}`} className="block group border-b border-slate-100 pb-4 last:border-0 last:pb-0">
                  <h4 className="text-sm font-bold text-slate-800 group-hover:text-blue-600 line-clamp-2">
                    {rj.title}
                  </h4>
